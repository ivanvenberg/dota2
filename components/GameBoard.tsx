...
export default function GameBoard({ playerId, isHost }: { playerId: string; isHost: boolean }) {
  ...
  const applyPrize = useMutation(({ storage }, name: string, emoji: string, desc: string) => {
    ...
    storage.get("chat").push({
      ...
      message: `🏆 ${me.name} got: ${emoji} ${name}!`, type: "game-event", timestamp: Date.now(),
    });
  }, [playerId]);
  ...
  const applyDuelSetup = useMutation(({ storage }, targetId: string, question: Question) => {
    ...
    storage.get("chat").push({
      id: Date.now().toString(), playerId: "system", playerName: "System",
      message: `🥊 DUEL! ${me.name} vs ${target.name}!`, type: "game-event", timestamp: Date.now(),
    });
  }, [playerId]);
  ...
  const handleExpertChallenge = useMutation(({ storage }, correction: string) => {
    ...
    storage.get("game").set("expertChallenge", {
      challengerId: playerId, challengerName: me.name,
  ...
  const handleRoll = useCallback(async () => {
      ...
      if (others.length > 0) {
        ...
        try {
          const res = await fetch("/api/question", {
            ...
            body: JSON.stringify({ type: "duel", heroes: players.map((p: any) => p.heroId) }),
          });
    ...
    try {
      const res = await fetch("/api/question", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: sqType, heroes: players.map((p: any) => p.heroId) }),
      });
      const { question } = await res.json() as { question: Question };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 mb-4 flex flex-wrap items-center gap-4 justify-between">
          <div>
            ...
            <span className="text-yellow-400 font-black text-2xl">{round}</span>
            <span className="text-gray-600 text-sm">/{maxRounds}</span>
          </div>
          <p className="flex-1 text-center text-white font-semibold text-sm px-4">{lastEvent}</p>
          {diceRoll != null && (
            <div className={`text-4xl select-none ${diceAnim ? "dice-roll" : ""}`}>
              {DICE_FACES[diceRoll - 1]}
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 bg-gray-900 border border-gray-700 rounded-2xl p-4">
            <div className="grid grid-cols-5 gap-1.5" style={{ aspectRatio: "1" }}>
              {Array.from({ length: 25 }, (_, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="board-square p-1 min-h-[3rem]"
            ...
            {isMyTurn && phase === "playing" && !loadingQ && (
              <button
                onClick={handleRoll}
                className="mt-4 w-full bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white font-black py-4 rounded-xl text-xl transition-all hover:scale-105 active:scale-95"
              >
                🎲 ROLL THE DICE!
              </button>
            )}
            ...
            <div className="bg-gray-900 border border-gray-700 rounded-2xl p-4">
              ...
              <div className="flex flex-col gap-2">
                {[...players].sort((a: any, b: any) => b.score - a.score).map((player: any, idx: number) => (
                  <div
                    key={player.id}
                    className={`flex items-center gap-2 p-2 rounded-lg ${player.id === currentPlayerId ? "bg-yellow-950/40 border border-yellow-800" : "bg-gray-800"}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex gap-1">
                        ...
                        {player.isBanned && <span className="text-[9px] text-red-400">🚫</span>}
                      </div>
                    </div>
                ...
                {chat.map((msg: any) => (
                  <p
                    ...
                    className={`text-xs leading-relaxed ${
                      ...
                      msg.type === "expert" ? "text-cyan-300" : "text-gray-400"
                    }`}
                  >
                    {msg.message}
                  </p>
