...
export default function GameBoard({ playerId, isHost }: { playerId: string; isHost: boolean }) {
  ...
  const currentPlayerIndex = useStorage((root) => root.game.get("currentPlayerIndex"));
  const playerOrder = useStorage((root) => root.game.get("playerOrder"));
  const round = useStorage((root) => root.game.get("round"));
  const maxRounds = useStorage((root) => root.game.get("maxRounds"));
  const diceRoll = useStorage((root) => root.game.get("diceRoll"));
  const currentQuestion = useStorage((root) => root.game.get("currentQuestion"));
  const currentDuel = useStorage((root) => root.game.get("currentDuel"));
  const expertChallenge = useStorage((root) => root.game.get("expertChallenge"));
  const lastEvent = useStorage((root) => root.game.get("lastEvent"));
  const chat = useStorage((root) => { const a: any[] = []; root.chat.forEach((m) => a.push(m)); return a.slice(-20); });
  const players = useStorage((root) => { const a: any[] = []; root.players.forEach((v) => a.push(v)); return a; });
  ...
  const advanceTurn = useMutation(({ storage }) => {
    ...
    if (newRound > maxR) {
      ...
      allPlayers.forEach((p) => { if (p.score > top) { top = p.score; winners = [p.id]; } else if (p.score === top) winners.push(p.id); });
    ...
    for (let i = 0; i < order.length; i++) {
      ...
      if (np?.isBanned) { const left = np.banRoundsLeft - 1; pm.set(order[skip], { ...np, banRoundsLeft: left, isBanned: left > 0 }); skip = (skip + 1) % order.length; }
      else break;
    }
  ...
  const applyRoll = useMutation(({ storage }, roll: number, sqType: string, newPos: number) => {
    const ap = storage.get("players"); const me = ap.get(playerId); if (!me) return;
  ...
  const applyPrize = useMutation(({ storage }, name: string, emoji: string, desc: string) => {
    const me = storage.get("players").get(playerId); if (!me) return;
    storage.get("game").set("lastEvent", `🏆 ${me.name} hit the Prize Zone! ${emoji} ${name} — ${desc}`);
  ...
  const applyDuelSetup = useMutation(({ storage }, targetId: string, question: Question) => {
    const ap = storage.get("players"); const me = ap.get(playerId); const target = ap.get(targetId);
    if (!me || !target) return;
    const game = storage.get("game");
    game.set("phase", "duel");
  ...
  const handleExpertChallenge = useMutation(({ storage }, correction: string) => {
    const me = storage.get("players").get(playerId); if (!me?.isExpert) return;
  ...
  const resolveExpertChallenge = useMutation(({ storage }, upheld: boolean) => {
    if (!isHost) return;
    const game = storage.get("game"); const ch = game.get("expertChallenge"); if (!ch) return;
    const ap = storage.get("players"); const expert = ap.get(ch.challengerId);
    if (upheld) {
  ...
  const handleRoll = useCallback(async () => {
    ...
    setDiceAnim(true); setTimeout(() => setDiceAnim(false), 700);
    const roll = rollDice();
  ...
  const handleAnswerAndAdvance = useCallback((idx: number, correct: boolean) => {
    handleAnswer(idx, correct); setTimeout(() => advanceTurn(), 3500);
  }, [handleAnswer, advanceTurn]);

  const handleResolveAndAdvance = useCallback((upheld: boolean) => {
    resolveExpertChallenge(upheld); setTimeout(() => advanceTurn(), 3000);
          ...
          <div className="lg:col-span-2 bg-gray-900 border border-gray-700 rounded-2xl p-4">
            <div className="grid grid-cols-5 gap-1.5" style={{ aspectRatio: "1" }}>
              {Array.from({ length: 25 }, (_, idx) => {
                ...
                if (!isPerimeter) { if (idx === 12) return <div key={idx} className="flex items-center justify-center text-3xl">⚔️</div>; return <div key={idx} className="bg-gray-800/20 rounded" />; }
            ...
            {isMyTurn && phase === "playing" && !loadingQ && (
              <button onClick={handleRoll} className="mt-4 w-full bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white font-black py-4 rounded-xl text-xl transition-all hover:scale-105 active:scale-95">
                🎲 ROLL THE DICE!
              </button>
            )}
            ...
            {loadingQ && (
              <div className="mt-4 text-center text-gray-400 py-2 flex items-center justify-center gap-2">
                <div className="animate-spin">⚔️</div> Summoning a question from the Ancients...
              </div>
            )}
          </div>
