...
export default function Results({ playerId }: { playerId: string }) {
  const players = useStorage((root) => { const a: any[] = []; root.players.forEach((v) => a.push(v)); return a.sort((a, b) => b.score - a.score); });
  ...
  function getRec(player: any): string {
    ...
    if (acc >= 60) return "Solid! Re-watch some replay VODs on counterpicking.";
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-950 to-gray-900 p-6">
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8">
          {winnerIds?.includes(playerId) ? (<><div className="text-6xl mb-3 animate-bounce">🏆</div><h1 className="text-4xl font-black text-yellow-400 mb-2">GG WP!</h1><p className="text-gray-300">You&apos;re the Dota 2 Knowledge Champion!</p></>) : (<><div className="text-6xl mb-3">⚔️</div><h1 className="text-4xl font-black text-gray-300 mb-2">Game Over!</h1></>)}
        </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-black text-white text-lg">{player.funnyName ? <><span className="text-orange-400">{player.funnyName}</span><span className="text-gray-500 text-sm ml-1">({player.name})</span></> : player.name}</span>
