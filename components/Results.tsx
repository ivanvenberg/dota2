...
export default function Results({ playerId }: { playerId: string }) {
  ...
  const winnerIds = useStorage((root) => root.game.winnerIds) ?? [];
  const MEDALS = ["🥇", "🥈", "🥉"];
      ...
      <div className="max-w-3xl mx-auto">
        <div className="text-center mb-8">
          {winnerIds.includes(playerId) ? (
            <>
              ...
              <p className="text-gray-300">You&apos;re the Dota 2 Knowledge Champion!</p>
            </>
          ) : (
            <>
                ...
                <div key={player.id} className="bg-gray-800 rounded-xl p-4 border border-gray-700">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="w-32 h-2 bg-gray-700 rounded-full mt-1">
                        <div className="h-2 rounded-full" style={{ width: `${acc}%`, backgroundColor: acc >= 70 ? "#22c55e" : acc >= 40 ? "#f59e0b" : "#ef4444" }} />
                      </div>
                    </div>
                  ...
                  {hero && (
                    <div className="mt-2 flex flex-wrap gap-1">
                      ...
                      {hero.counters.slice(0, 3).map((c) => (
                        <span key={c} className="text-xs bg-red-950 border border-red-900 text-red-400 px-2 py-0.5 rounded-lg">⚔️ {c}</span>
                      ))}
                    </div>
                  )}
                </div>
        ...
        <div className="text-center">
          <button
            ...
            className="bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white font-black py-4 px-10 rounded-xl text-xl transition-all hover:scale-105"
          >
            🎮 Play Again
          </button>
