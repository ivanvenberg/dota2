...
export default function QuestionCard({ question, isMyTurn, myPlayer, onAnswer, onExpertChallenge, expertChallenge, isHost, onResolveChallenge }: Props) {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => setTimeLeft((v) => { if (v <= 1) { clearInterval(t); setRevealed(true); onAnswer(-1, false); return 0; } return v - 1; }), 1000);
      ...
      <div className={`bg-gradient-to-b ${bg} border-2 rounded-2xl p-6 w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto`}>
        ...
        <div className="flex flex-col gap-2 mb-4">
          {question.options.map((opt, idx) => {
            ...
            if (revealed) { if (idx === question.correctIndex) cls = "bg-green-900 border-green-500 text-green-200"; else if (idx === selected) cls = "bg-red-900 border-red-500 text-red-200"; else cls = "bg-gray-900 border-gray-800 text-gray-500"; }
            return (
        ...
        {myPlayer?.isExpert && !expertChallenge && (
          <div className="border-t border-gray-700 pt-4">
            {!showExpertInput ? (
              <button onClick={() => setShowExpertInput(true)} className="text-sm bg-cyan-900 hover:bg-cyan-800 border border-cyan-700 text-cyan-300 px-4 py-2 rounded-lg transition-colors">🎓 Challenge this answer as Expert</button>
            ) : (
