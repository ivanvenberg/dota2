...
export default function DuelModal({ duel, playerId, players, advanceTurn }: { duel: DuelState; playerId: string; players: PlayerData[]; advanceTurn: () => void; }) {
  ...
  const submitAnswer = useMutation(({ storage }, answerIndex: number) => {
    const game = storage.get("game"); const cur = game.get("currentDuel"); if (!cur) return;
    const updated = { ...cur };
    if (playerId === cur.player1Id) updated.p1Answer = answerIndex;
    else if (playerId === cur.player2Id) updated.p2Answer = answerIndex;
    const p1Done = updated.p1Answer !== null; const p2Done = updated.p2Answer !== null;
    if (p1Done && p2Done) {
      const p1c = updated.p1Answer === cur.correctIndex; const p2c = updated.p2Answer === cur.correctIndex;
      ...
      if (winnerId && winnerId !== "tie") {
        const winner = ap.get(winnerId); if (winner) ap.set(winnerId, { ...winner, score: winner.score + 400, answeredCorrectly: winner.answeredCorrectly + 1 });
        const loserId = winnerId === cur.player1Id ? cur.player2Id : cur.player1Id;
        const loser = ap.get(loserId);
        if (loser && !loser.funnyName) { const funny = getRandomFunnyNickname(Array.from(ap.values()).map((p) => p.funnyName).filter((n): n is string => !!n)); ap.set(loserId, { ...loser, funnyName: funny, answeredWrong: loser.answeredWrong + 1 }); }
      }
      const msg = winnerId === "tie" ? "🤝 DUEL TIE!" : winnerId ? `🏆 ${ap.get(winnerId)?.name ?? "?"} WINS THE DUEL! +400 pts!` : "🤝 Nobody answered";
  ...
  function handleAnswer(idx: number) {
    ...
    setMyAnswered(true); submitAnswer(idx);
      ...
      <div className="bg-gradient-to-b from-orange-950 to-gray-950 border-2 border-orange-700 rounded-2xl p-6 w-full max-w-2xl shadow-2xl">
        ...
        <div className="flex flex-col gap-2">
          {duel.options.map((opt, idx) => {
            ...
            if (myAnswer !== null || isDone) { if (idx === duel.correctIndex) cls = "bg-green-900 border-green-500 text-green-200"; else if (idx === myAnswer && idx !== duel.correctIndex) cls = "bg-red-900 border-red-500 text-red-200"; else cls = "bg-gray-900 border-gray-800 text-gray-500"; }
